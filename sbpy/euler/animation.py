...
def animate_pressure(grid, P, dt):
    ...
    anim = animation.FuncAnimation(
        fig, update, fargs=(plot,), interval=1000 * dt, blit=True
    )
    plt.show()
...
def animate_solution(grid, U, V, dt, save_gif=False, repeat=True):
    ...
    s_plot = ax.pcolormesh(X, Y, S[0], shading="gouraud")
...
def animate_speed(grid, U, V, dt):
    ...
    anim = animation.FuncAnimation(
        fig, update, fargs=(s_plot,), interval=1000 * dt, blit=True
    )
    plt.show()
...
def plot_speed(grid, U, V):
    ...
    X, Y = grid.get_block(0)
    S = np.sqrt(np.array(U) ** 2 + np.array(V) ** 2)
    s_plot = ax.pcolormesh(
        X, Y, S, edgecolors="face", antialiased=True, shading="gouraud"
    )
    s_min = np.min(np.array(S).flatten())
    s_max = np.max(np.array(S).flatten())
    s_plot.set_clim([s_min, s_max])
